<script lang="ts">
	...
	export let tournament: Tournament & { title?: string };

	let activeTab = 'BRIEF';
	let details: Tournament | null = null;
	let loading = true;

	$: bgImage = tournament?.imageUrl || '';
	$: statusText =
		tournament?.status === 1 ? 'RECRUITING' : tournament?.status === 3 ? 'LIVE' : 'ARCHIVED';
...
<div class="details-container">
	...
	<div class="details-hero">
		...
		<div class="hero-content">
			<button class="back-btn" on:click={goBack}>&larr; ABORT MISSION</button>
			<div class="tags">
				<span class="status-tag">{statusText}</span>
				<span class="id-tag">OP-ID: {tournament.id.substring(0, 8)}</span>
			</div>
	...
	<div class="tabs-container">
		<div class="tabs">
			<button class:active={activeTab === 'BRIEF'} on:click={() => (activeTab = 'BRIEF')}
				>MISSION BRIEF</button
			>
			<button class:active={activeTab === 'BRACKET'} on:click={() => (activeTab = 'BRACKET')}
				>BRACKET INTEL</button
			>
			<button class:active={activeTab === 'ROSTER'} on:click={() => (activeTab = 'ROSTER')}
		...
		{:else if activeTab === 'BRIEF'}
			<div class="panel brief-panel">
				<div class="brief-content">
					...
					<p class="desc">
						Get ready for the ultimate showdown. This tournament pits the best Walkers against each
						other. Prepare your loadouts and sync your drones.
					</p>

					<div class="stats-hud">
						<div class="hud-item entry">
							<span class="hud-label">ENTRY FEE</span>
							<span class="hud-val">{details?.entryFee || 0} WC</span>
						</div>
						<div class="hud-item prize">
							<span class="hud-label">PRIZE POOL</span>
							<span class="hud-val gold">{details?.prizePool || 0} WC</span>
						</div>
						<div class="hud-item players">
							<span class="hud-label">OPERATIVES</span>
							<span class="hud-val">{details?.maxParticipants || 16} MAX</span>
						</div>
					</div>
				</div>
				...
				<div class="action-area">
					...
					<p class="terms">By joining, you accept the Walker Protocol.</p>
				</div>
			</div>
		{:else if activeTab === 'BRACKET'}
			<div class="panel">
				<h3 class="panel-header">TOURNAMENT BRACKET</h3>
				<div class="bracket-placeholder">NO ACTIVE BRACKET DATA FOUND.</div>
			</div>
